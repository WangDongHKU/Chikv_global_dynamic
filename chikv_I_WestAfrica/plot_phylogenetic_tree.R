...
# 读取树文件 (NEXUS格式)
cat("正在读取树文件...\n")
tree <- read.nexus("West_African_aln_2MCC.tree")

# 读取元数据
cat("正在读取元数据...\n")
metadata <- read.delim("West_African_with_coordinates.tsv", sep = "\t", stringsAsFactors = FALSE)
...
if (use_ggtree) {
  ...
  if (use_ggtree) {
    ...
    p <- ggtree(tree, layout = "rectangular", size = 0.3) %<+% tip_data_full +
      ...
      labs(title = "Phylogenetic Tree of CHIKV I-WestAfrica Genotype")
  ...
  # 添加标题
  title("Phylogenetic Tree of CHIKV I-WestAfrica Genotype", cex.main = 1.5)
