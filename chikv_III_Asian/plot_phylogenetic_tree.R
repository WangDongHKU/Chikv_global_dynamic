...
setwd("/scr/u/dongw21/Chikungunya/chikv_III_Asian")
...
# 读取树文件 (NEXUS格式)
cat("正在读取树文件...\n")
tree <- read.nexus("Asian_alnMCC1.tree")

# 读取元数据
cat("正在读取元数据...\n")
metadata <- read.delim("Asian_with_coordinates.tsv", sep = "\t", stringsAsFactors = FALSE)
...
if (use_ggtree) {
  ...
  if (use_ggtree) {
    ...
    p <- ggtree(tree, layout = "rectangular", size = 0.3) %<+% tip_data_full +
      ...
      labs(title = "Phylogenetic Tree of CHIKV III-Asian Genotype")
  ...
  # 添加标题
  title("Phylogenetic Tree of CHIKV III-Asian Genotype", cex.main = 1.5)
